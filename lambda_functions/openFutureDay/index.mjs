...
export const handler = async (event) => {
  ...
  try {
     ...
     const [closedDay] = await connection.execute("SELECT * FROM ClosedDays WHERE date = ? AND rid = ?", [`${year}-${month}-${day}`, rid])
     if (closedDay.length === 0) return errorResponse(400, "This day is not closed")
     await connection.execute("DELETE FROM ClosedDays WHERE date = ? AND rid = ?",[`${year}-${month}-${day}`, rid])

     console.log("Closed Day Reopened")
 
     return {
       statusCode: 200,
       body: JSON.stringify({ message: "Closed day reopened successfully" }),
     }
   } catch (error) {
     console.error(error)
