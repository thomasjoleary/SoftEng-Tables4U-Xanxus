...
export const handler = async (event) => {
    ...
    try {
        ...
        const { year, month, day, time, guests, name } = JSON.parse(event.body || "{}");
        console.log(`Received data: year=${JSON.stringify(year)}, month=${JSON.stringify(month)}, day=${JSON.stringify(day)}, time=${JSON.stringify(time)}, guests=${JSON.stringify(guests)}, name=${JSON.stringify(name)}`);
        ...
        if (!name) return errorResponse(400, "No restaurant name entered.")
        ...
        const restQuery = `
        SELECT r.rid, r.name, r.address, t.tid, t.seats
...
`;
        ...
        const [results] = await connection.execute(restQuery, [
            mysqlDate,     // for closed day check
        ...
        if (results.length === 0) {
            return errorResponse(400, "No tables available for your restaurant, day, time or amount of guests.");
        }
